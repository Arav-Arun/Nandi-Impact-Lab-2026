...
class SubscriberIn(BaseModel):
    channel: str                       # sms | whatsapp | telegram | email
    address: str                       # phone | chat id | email
...
async def add_subscriber(body: SubscriberIn, session: AsyncSession = Depends(get_session)):
    if body.channel not in {"sms", "whatsapp", "telegram", "email"}:
        raise ApiError("BAD_CHANNEL", "channel must be sms|whatsapp|telegram|email")
